...
public class consumer {
    ...
    private String topic = "wushang_topic";
    ...
    public void insterDB(List<String> userlist, String name) {
        ...
        if (!userlist.contains(name)) {
            ...
            book.setBookdes("rocketmq-"+name);
            bookMapper.insertBook(book);
            userlist.add(name);
            int key = name.length();
            System.out.println("redis set>>>>>>>>>>key：" + key + " and value=" + name.toString());
            redisTemplate.opsForValue().set(String.valueOf(key), name.toString());
            providerService.toRabbitmq(name);
        }
    }
}